...
*/
    ...
    */
    ...
    let x_data = training_records.outer_iter().map(|row| row.to_vec().into_iter().map(|x| x as f32).collect::<Vec<f32>>()).map(|x| x.as_slice()).collect::<Vec<&[f32]>>();

    let x = DenseMatrix::<f32>::from_2d_array(x_data.as_slice());
