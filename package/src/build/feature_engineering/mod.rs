...
pub fn get_features(text: &String) -> Vec<f64> {
    ...
    features.push(super::naive_bayes::categorical_nb_model_predict(vec![text.clone()]).unwrap()[0] as f64); // 2
    features.push(super::naive_bayes::gaussian_nb_model_predict(vec![text.clone()]).unwrap()[0] as f64); // 1
