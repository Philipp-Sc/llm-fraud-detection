...
Trained on text embeddings generated by BERT.
    ...
    Note: A KNN remembers the whole dataset. (The model size is about 500MB)
...
Takes in the predictions from above models as latent input variables and predicts spam/ham.
<details>
...
The KNN Regressor on the text embeddings functions as (smart) memory of the spam/ham dataset.    
The Random Forest Regressor on the fraud indicators represents a reasoning method to determine spam/ham.

Together both predictors complement each other (semantic search + reasoning).
